...
import { 
  ...
  AlertTriangle, Barcode, Check, FileWarning, WandSparkles
} from 'lucide-react';
...
export function ReceiptFormModal({ isOpen, onClose, onSubmit }: ReceiptFormModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
       Promise.all([api.getProducts(), api.getSupplierOrders(), api.getSuppliers()])
         .then(([prodRes, ordRes, supRes]) => {
            setProducts(prodRes);
            setOrders(ordRes);
            setSuppliers(supRes);
         })
  ...
  useEffect(() => {
    ...
    if (fromOrder === 'oui' && orderId) {
      ...
      if (order) {
        ...
        setLines((order.lignes || []).map((l: any) => {
        ...
        setReferenceBL(`BL-${order.reference}`);
      }
  ...
  }, [fromOrder, orderId, isOpen, orders, products, serialNumbersEnabled]);
  ...
  const handleCloseAttempt = () => {
    if (hasUnsavedChanges && !isSuccess && !isProcessing) {
  ...
  const handleDemoFill = () => {
    const receivableOrders = orders.filter((item) => ['Validée', 'Envoyée', 'Partiellement reçue'].includes(item.statut));
    const order = receivableOrders.find((item) =>
      (item.lignes || []).some((line: any) => products.find((product) => product.id === line.productId)?.requiresSerialNumber === true)
    ) || receivableOrders[0] || orders[0];
    if (!order) return;
    ...
    setReferenceBL(`BL-DEMO-${demoCode}`);
    setReferenceFacture(`FAC-DEMO-${demoCode}`);
    setDepot('Dépôt Principal');
    setResponsable('Magasinier');
    setCommentaire(`Réception de démonstration liée à la commande ${order.reference}. Contrôle quantitatif et qualitatif effectué.`);
    setLines((order.lignes || []).map((line: any, index: number) => {
      const product = products.find((item) => item.id === line.productId);
      const qtyToReceive = Math.max(0, line.qteCmd - (line.qteRecue || 0));
      const serialized = product?.requiresSerialNumber === true && serialNumbersEnabled;
      return {
        id: `demo-receipt-${line.id || line.productId}-${index}`,
        orderLineId: line.id,
        productId: line.productId,
        productName: line.productName,
        refFournisseur: line.refFournisseur,
        qteCmd: line.qteCmd,
        qteRecue: qtyToReceive,
        prixU: line.prixU,
        emplacement: 'loc1',
        etat: 'conforme',
        serials: serialized
          ? Array.from({ length: qtyToReceive }, (_, serialIndex) => `SN-${product.reference}-${demoCode}-${String(serialIndex + 1).padStart(3, '0')}`).join('\n')
          : '',
  ...
  const handleSelectProduct = (lineId: string, productId: string) => {
    ...
    if (product) {
      updateLine(lineId, {
        ...
        prixU: 0,
        isSerialized: product.requiresSerialNumber && serialNumbersEnabled
      });
  ...
  const handleSaveDraft = () => {
     ...
     const payload = { fromOrder, orderId, fournisseurId, dateReception, referenceBL, referenceFacture, depot, responsable, statut: 'Brouillon', commentaire, lines };
     setIsProcessing(true);
     api.createSupplierReceipt(payload).then(() => {
       setIsProcessing(false);
       onSubmit({ statut: 'Brouillon' });
     }).catch(console.error);
  };
  ...
  const handleValidate = async () => {
    ...
    const payload = { fromOrder, orderId, fournisseurId, dateReception, referenceBL, referenceFacture, depot, responsable, statut: 'Validée', commentaire, lines };

    try {
      await api.createSupplierReceipt(payload);
      
      setIsSuccess(true);
      setTimeout(() => {
        setIsProcessing(false);
        onSubmit({ statut: 'Validée' });
      }, 2000);
    } catch(err) {
      console.error(err);
      setIsProcessing(false);
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-md shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleDemoFill} disabled={!orders.length || isProcessing} className="flex items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-bold uppercase tracking-wider text-indigo-700 hover:bg-indigo-100 disabled:opacity-40">
              ...
              <div className="p-6 space-y-8">
                      ...
                      {fromOrder === 'oui' ? (
                        <div>
                          ...
                          <CustomSelect
                            ...
                            options={[{ value: '', label: 'Sélectionner une commande' }, ...orders.map(o => ({ value: o.id, label: `${o.reference} - ${o.fournisseurName} - ${o.statut}`}))]}
                          />
                        </div>
                      ) : (
                        <div>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div className={cn("p-4 rounded-xl border shadow-sm transition-all", stats.ecarts > 0 ? "bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800" : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800")}>
              ...
              <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
                ...
                >
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
                    Vous avez des données non enregistrées. Vos modifications seront perdues.
                  </p>
